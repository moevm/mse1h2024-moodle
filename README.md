...
### Варинт 1. Основной при работе с devtools (с помощью которого можно отследить весь функционал собираемой статистики с moevm) - не поднимается moodle и mariadb - порты оставлены открытыми, чтобы можно было отправлять данные статистики с moevm 
1. Сборка проекта
...
7. Открыть файл moodle_stat_tracker_without_script.html
...
### Вариант 2. Требует проверки от администратора moevm Основной при работе с панелью администратора на moevm (с помощью которого можно отследить весь функционал собираемой статистики с moevm) - не поднимается moodle и mariadb - порты оставлены открытыми, чтобы можно было отправлять данные статистики с moevm 
Повторить действия 1 - 5 из Варианта 1
6. Открыть файл moodle_stat_tracker.html
