...
Для запуска "чистового варианта", где не поднимается moodle и mariadb, а также закрыты порты для mongodb и backend, необходимо использовать `docker-compose.prod.yaml.`

Поднимать контейнеры командой: 
```bash
...
Настройка происходит с помощью изменения значений в полях куска кода трекера, который расположен выше.
 ...
 - `numberActionsToSend` $-$ Определяет число действий пользователя которые нужно отправлять за итерацию. По умолчанию отсылается по 15 действий пользователя, но можно изменить это значение.
